...
def test():
    ...
    if 1:
        ...
        outlier_idxs = all_idxs[:n_outliers]  # 选择打乱后的前100个索引作为局外点的索引。
    ...
    # setup model  设置模型
    all_data = np.hstack((A_noisy, B_noisy))  # 形式([Xi,Yi]....) shape:(500,2)500行2列 / 将输入数据和输出数据水平拼接，形成一个包含所有数据的数组。
    input_columns = range(n_inputs)  # 数组的第一列x:0
    output_columns = [n_inputs + i for i in range(n_outputs)]  # 数组最后一列y:1
    debug = False
    model = LinearLeastSquareModel(input_columns, output_columns, debug=debug)  # 类的实例化:用最小二乘生成已知模型

    # 使用scipy.linalg.lstsq函数进行线性最小二乘法拟合
    # all_data[:, input_columns]为输入数据
    # all_data[:, output_columns]为输出数据
    # resids：残差。
    # rank：矩阵的秩。
    # s：奇异值，
    ...
    if 1:
        ...
        sort_idxs = np.argsort(A_exact[:, 0])  # 对A_exact数组的第一列进行排序，并返回排序后的索引。
        A_col0_sorted = A_exact[sort_idxs]  # 秩为2的数组 / 根据排序后的索引重新排列A_exact数组，得到A_col0_sorted

        if 1:
            pylab.plot(A_noisy[:, 0], B_noisy[:, 0], 'k.', label='data')  # 使用黑色点（'k.'）绘制A_noisy和B_noisy数组的第一列，表示原始数据点
            pylab.plot(A_noisy[ransac_data['inliers'], 0], B_noisy[ransac_data['inliers'], 0], 'bx',
                       label="RANSAC data")  # 使用蓝色叉号（'bx'）绘制RANSAC算法识别的内点（inliers），这些点用于拟合模型
        else:
            # 使用黑色点（'k.'）绘制非局外点（non_outlier_idxs）。 使用红色点（'r.'）绘制局外点（outlier_idxs）。
            pylab.plot(A_noisy[non_outlier_idxs, 0], B_noisy[non_outlier_idxs, 0], 'k.', label='noisy data')
            pylab.plot(A_noisy[outlier_idxs, 0], B_noisy[outlier_idxs, 0], 'r.', label='outlier data')

        pylab.plot(A_col0_sorted[:, 0],
                   np.dot(A_col0_sorted, ransac_fit)[:, 0],
                   label='RANSAC fit')  # 得到拟合线，并绘制
        pylab.plot(A_col0_sorted[:, 0],
                   np.dot(A_col0_sorted, perfect_fit)[:, 0],
                   label='exact system')  # 得到完美拟合线，并绘制
        pylab.plot(A_col0_sorted[:, 0],
                   np.dot(A_col0_sorted, linear_fit)[:, 0],
                   label='linear fit')  # 得到线性拟合线，并绘制
        pylab.legend()  # 添加图例，显示不同数据点和拟合线的标签。
        pylab.show()  # 显示绘制的图形。
