...
# mnist.load_data() 在机器学习和深度学习中用于加载著名的MNIST手写数字数据集
...
# plt.cm.binary是一种预定义的颜色映射，它将较低的数值映射为黑色，较高的数值映射为白色，这非常适合显示二值图像或灰度图像
...
network.add(layers.Dense(10, activation='softmax')) # 10个神经元，使用softmax激活函数

# 编译模型，指定了训练过程中使用的优化器、损失函数和评估指标
# RMSprop（Root Mean Square Propagation）是一种优化算法，用于调整网络的权重以最小化损失函数。它是一种自适应学习率优化方法
# loss='categorical_crossentropy'：是多分类问题中常用的损失函数。它衡量的是模型输出的概率分布与真实标签的概率分布之间的差异
# metrics=['accuracy']：这指定了在训练过程中要计算的评估指标，这里是准确率
...
print("before change:", test_labels[1])
# 独热编码是一种将分类变量转换为机器学习算法可以更好处理的形式的方法。在独热编码中，每个类别被表示为一个二进制向量，除了表示该类别的一个位置为1外，其余位置都为0。
train_labels = to_categorical(train_labels)  # 转换标签为独热编码
test_labels = to_categorical(test_labels)
print("after change: ", test_labels[1])
...
识别效果与硬件有关（CPU/GPU）.
'''
# network.evaluate是Keras中用于评估模型性能的方法。它接受测试数据、测试标签和可选的参数verbose
# test_loss：这是模型在测试数据集上的损失值。
# test_acc：这是模型在测试数据集上的准确率
...
plt.show()
test_images = test_images.reshape((10000, 28 * 28))
# network.predict是Keras中用于对输入数据进行预测的方法
