...
batch_size = 100  # 批次的样本数量
num_examples_for_eval = 10000  # 样本总数
...
# 创建x和y_两个placeholder 占位符，用于在训练或评估时提供输入的数据和对应的标签值。
# 要注意的是，由于以后定义全连接网络的时候用到了batch_size，所以x中，第一个参数不应该是None，而应该是batch_size
x = tf.placeholder(tf.float32, [batch_size, 24, 24, 3])
y_ = tf.placeholder(tf.int32, [batch_size])

# kernel1和kernel2：两个卷积层的权重，使用variable_with_weight_loss函数创建。
# conv1和conv2：应用卷积操作。
# bias1和bias2：卷积层的偏置项。
# relu1和relu2：应用ReLU激活函数。
# pool1和pool2：应用最大池化
