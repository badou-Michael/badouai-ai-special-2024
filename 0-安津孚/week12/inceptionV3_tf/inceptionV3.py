...
def InceptionV3(input_shape=[299, 299, 3],
    ...
    # conv2d_bn：是一个自定义函数，组合了卷积层（Conv2D）、批量归一化层（BatchNormalization）和ReLU激活层（Activation）。模型的初步处理
    ...
    # 全连接层，输出类别数量为 classes，使用softmax激活函数进行多类别分类
