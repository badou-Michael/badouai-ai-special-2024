...
def MobileNet(input_shape=[224, 224, 3],
    ...
    # 7,7,1024 -> 1,1,1024
    # GlobalAveragePooling2D：全局平均池化层，用于减少参数数量并提取特征。
    # Reshape：将特征图重塑为特定的形状，这里用于将全局平均池化后的输出重塑为 (1, 1, 1024)。
    # Dropout：dropout层，用于正则化。
    # Conv2D：一个1x1卷积层，用于将特征图转换为类别数 classes。
    # Activation：softmax激活函数，用于多类别分类。.
...
def _conv_block(inputs, filters, kernel=(3, 3), strides=(1, 1)):
    x = Conv2D(filters, kernel,
               padding='same',
               use_bias=False,  # use_bias=False：指定卷积层不使用偏置项
               strides=strides,
               name='conv1')(inputs)
