...
from keras.layers import Input, Conv2D, BatchNormalization, Activation, DepthwiseConv2D, GlobalAveragePooling2D, Reshape, Dropout
...
def MobileNet(input_shape=(224,224,3), depth_multiplier = 1, dropout = 1e-3, classes = 1000):
    ...
    # 7,7,1024 -> 1,1,1024
    x = GlobalAveragePooling2D()(x)  #  (batch_size, 1024) 只保留channel维度的值
    x = Reshape((1, 1, 1024), name='reshape_1')(x) # 转为 (batch_size, 1, 1, 1024), 以适配卷积操作
    x = Dropout(dropout, name='dropout')(x)
    x = Conv2D(classes, (1, 1),padding='same', name='conv_preds')(x)  # 转为 (batch_size, 1, 1, 1000)
    x = Activation('softmax', name='act_softmax')(x)
    x = Reshape((classes,), name='reshape_2')(x)  # 转换为标准分类任务格式 (batch_size, classes)
    # 注: Conv2D 和 第二个Reshape可以替换成 Dense
