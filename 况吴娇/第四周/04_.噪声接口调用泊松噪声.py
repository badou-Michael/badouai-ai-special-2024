...
##util是skimage（scikit-image）库中的一个模块，它包含了一系列有用的图像处理工具和函数。
# 这些工具和函数可以用于执行各种常见的图像操作，比如图像变换、图像分割、图像形态学操作等。
# random_noise: 向图像添加随机噪声。

img=cv2.imread('lenna.png')
poisson_img=util.random_noise(img,mode='salt') #s&p\salt
cv2.imshow('source',img)

cv2.imshow('poisson_img',poisson_img)
#cv.imwrite('poisson_img.png',poisson_img)
