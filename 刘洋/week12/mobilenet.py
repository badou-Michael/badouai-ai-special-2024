...
def MobileNet(input_shape=[224, 224, 3],
    ...
    x = _depthwise_conv_block(x, 512, depth_multiplier, block_id=11)  # 14,14,512

    x = _depthwise_conv_block(x, 1024, depth_multiplier,strides=(2, 2), block_id=12)
    x = _depthwise_conv_block(x, 1024, depth_multiplier, block_id=13)  # 7,7,1024

    x = GlobalAveragePooling2D()(x)  # none，1024
    x = Reshape((1, 1, 1024), name='reshape_1')(x)  # 1,1,1024
    x = Dropout(dropout, name='dropout')(x)
    x = Conv2D(classes, (1, 1), padding='same', name='conv_preds')(x)  # 1,1,1000
    x = Activation('softmax', name='act_softmax')(x)
    x = Reshape((classes,), name='reshape_2')(x)  # none，1000

    inputs = img_input

    model = Model(inputs, x, name='mobilenet_1_0_224_tf')
    model_name = './weights/mobilenet_1_0_224_tf.h5'
...
def relu6(x):
    return relu(x, negative_slope=0.0, max_value=6, threshold=0.0)
...
if __name__ == '__main__':
    ...
    x0 = preprocess_input(x)
    print('input image shape:', x0.shape)

    preds = model.predict(x0)
    print(np.argmax(preds))
    print('predicted:', decode_predictions(preds, 1))  # top1
