...
class yolo:
    ...
    def _batch_normalization_layer(self, input_layer, name = None, training = True, norm_decay = 0.99, norm_epsilon = 1e-3):
        ...
        '''
        # bn_layer = tf.keras.layers.BatchNormalization()(input_layer)
    ...
    def _conv2d_layer(self, inputs, filters_num, kernel_size, name, use_bias = False, strides = 1):
        ...
        """
        # conv = tf.keras.layers.Conv2D(filters=filters_num, kernel_size=kernel_size, strides=(strides,strides), padding=('SAME' if strides == 1 else 'VALID'),name=name)(inputs)
