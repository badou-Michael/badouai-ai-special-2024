...
from common.path import dataset


class HomeworkW10NeuralNetWork(object):
    ...
    def train_w10(self, inputs_list, targets_list):
        """
        根据输入的训练数据更新节点链路权重
...
output_nodes = 10
learning_rate = 0.1
n = HomeworkW10NeuralNetWork(input_nodes, hidden_nodes, output_nodes, learning_rate)
# 读入训练数据
# open函数里的路径根据数据存储的路径来设定
training_data_file = open(os.path.join(dataset, 'mnist_train.csv'), 'r')
...
for e in range(epochs):
    ...
    for record in training_data_list:
        ...
        n.train_w10(inputs, targets)
test_data_file = open(os.path.join(dataset, 'mnist_test.csv'))
