...
def print_prob_w11(prob, file_path):
...
# 读取图片
img1 = load_image_w11("D:\workspace\data\\vgg16_data\\table.jpg")

# 对输入的图片进行resize，使其shape满足(-1,224,224,3)
inputs = tf.placeholder(tf.float32,[None,None,3])
resized_img = resize_image_w11(inputs, (224, 224))
...
# 载入模型
sess_w11 = tf.Session()
ckpt_filename = 'D:\workspace\data\\vgg16_data\\vgg_16.ckpt'
sess_w11.run(tf.global_variables_initializer())
saver = tf.train.Saver()
saver.restore(sess_w11, ckpt_filename)

# 最后结果进行softmax预测
pro = tf.nn.softmax(prediction)
pre = sess_w11.run(pro, feed_dict={inputs:img1})

# 打印预测结果
print("result: ")
print_prob_w11(pre[0], './synset.txt')
