...
def detect_face_12net(cls_prob, roi, out_side, scale, width, height, threshold):
    ...
    # plt.scatter(bb1[:,0],bb1[:,1],line_widths=1)
    # plt.scatter(bb2[:,0],bb2[:,1],line_widths=1,c='r')
    # plt.show()
...
def nms(rectangles, threshold):
    ...
    pick = []
    while len(i) > 0:
        xx1 = np.maximum(x1[i[-1]], x1[i[0:-1]])  # i[-1] have high_test prob score, i[0:-1]->others
