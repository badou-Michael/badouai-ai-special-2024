...
class Yolo:
    ...
    # l2 正则化
    @staticmethod
    def _batch_normalization_layer(input_layer, name=None, training=True, norm_decay=0.99, norm_epsilon=1e-3):
        """
        Introduction
        ------------
    ...
    # 这个就是用来进行卷积的
    @staticmethod
    def _conv2d_layer(inputs, filters_num, kernel_size, name, use_bias=False, strides=1):
        """
        Introduction
        ------------
    ...
    def yolo_inference(self, inputs, num_anchors, num_classes, training=True):
        ...
        with tf.variable_scope('yolo'):
            ...
            conv2d_60 = self._batch_normalization_layer(conv2d_60, name="batch_normalization_" + str(conv_index),
                                                        ...
                                                        norm_epsilon=self.norm_epsilon)
            conv_index += 1
            # un_sample_0 = 26,26,256
            un_sample_0 = tf.image.resize_nearest_neighbor(conv2d_60,
                                                           [2 * tf.shape(conv2d_60)[1], 2 * tf.shape(conv2d_60)[1]],
                                                           name='upSample_0')
            # route0 = 26,26,768
            route0 = tf.concat([un_sample_0, conv2d_43], axis=-1, name='route_0')
            ...
            conv2d_68 = self._batch_normalization_layer(conv2d_68, name="batch_normalization_" + str(conv_index),
                                                        ...
                                                        norm_epsilon=self.norm_epsilon)
            conv_index += 1
            # un_sample_1 = 52,52,128
            un_sample_1 = tf.image.resize_nearest_neighbor(conv2d_68,
                                                           [2 * tf.shape(conv2d_68)[1], 2 * tf.shape(conv2d_68)[1]],
                                                           name='upSample_1')
            # route1= 52,52,384
            route1 = tf.concat([un_sample_1, conv2d_26], axis=-1, name='route_1')
            # conv2d_75 = 52,52,255
