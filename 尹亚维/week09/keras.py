...
plt.imshow(digit, cmap=plt.cm.binary)  # 指定了颜色映射为二值颜色映射（黑白）
plt.show()
...
activation:激活函数
ReLU 激活函数定义为 (f(x) = max(0, x))，它将所有负值变为 0，正值保持不变。
Softmax：激活函数通常用于多分类问题，它将输出转换为概率分布，使得每个类别的概率值在 0 到 1 之间，并且所有类别的概率值之和为1
optimizer='rmsprop':指定了优化器为RMSprop。RMSprop 是一种自适应学习率的优化算法，适用于处理非稳态目标函数
loss='categorical_crossentropy':指定了损失函数为分类交叉熵（Categorical Crossentropy）。分类交叉熵通常用于多分类问题，它衡量的是预测概率分布与真实标签之间的差异。
metrics=['accuracy']: 指定了评估指标为准确率（Accuracy）。准确率是指模型预测正确的样本数占总样本数的比例。
'''
...
识别效果与硬件有关（CPU/GPU）.
verbose=1: 控制评估过程的详细程度。
verbose=1 表示在评估过程中显示进度条，verbose=0 表示不显示任何输出，verbose=2 表示每个epoch结束后显示一行输出。
est_loss: 评估过程中计算的损失值，对应于编译模型时指定的损失函数（例如 categorical_crossentropy）。
test_acc: 评估过程中计算的准确率，对应于编译模型时指定的评估指标（例如 accuracy）
'''
