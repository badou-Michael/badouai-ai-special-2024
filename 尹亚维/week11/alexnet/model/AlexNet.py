...
def AlexNet(input_shape=(224, 224, 3), output_shape=2):
    model = Sequential()
    # 使用步长为4x4，大小为11的卷积核对图像进行卷积，如果输出的特征层为96层，输出的shape为(55,55,96)；
    # 所建模型后输出为48特征层
    # padding = 'valid': 不进行填充，因此输出的特征图尺寸会小于输入图像尺寸。
    model.add(Conv2D(filters=48, kernel_size=(11, 11), strides=(4, 4), padding='valid', activation='relu', input_shape=input_shape))
