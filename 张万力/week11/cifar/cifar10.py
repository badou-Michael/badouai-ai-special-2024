...
cifar 模型组成
    输入层 图像大小24*24*3
    卷积层1 卷积核[5*5*3] 输出64个通道，池化后减半
    卷积层2 卷积核[5*5*64] 输出64个通道，池化后减半
    全连接层 拍扁成一维结构->FC1输出节点(384) -> FC2输出节点(192) ->  FC3输出节点10
    softmax 得到每个类别的概率
"""


#创建第一个卷积层 shape=(kh,kw,ci,co)  5e-2是个经验值，初始化卷积核的标准差
