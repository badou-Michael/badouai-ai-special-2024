...
def read_cifar10(file_queue):
    ...
    image_bytes=result.height * result.width * result.depth  #图片样本总元素数量，3072 字节的图像数据
    ...
    #因为该数组第一个元素是标签，所以我们使用strided_slice()函数将标签提取出来，并且使用tf.cast()函数将这一个标签转换成int32的数值形式，转换成int32而不是float32是因为类别索引通常是离散的整数值，整型有助于计算效率
...
def inputs(data_dir,batch_size,distorted):               #这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    filenames=[os.path.join(data_dir,"data_batch_%d.bin"%i)for i in range(1,6)]   #拼接地址 得到的地址为"Cifar_data\\cifar-10-batches-bin\\data_batch_1.bin"，此处是得到data_batch_1.bin~data_batch_6.bin的6个文件地址

    file_queue=tf.train.string_input_producer(filenames)     #根据已经有的文件地址创建一个文件队列 string_input_producer可以配合TensorFlow的Reader API（如 tf.TextLineReader、tf.FixedLengthRecordReader）按需读取文件内容
