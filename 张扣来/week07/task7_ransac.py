...
def ransac(data, model, n, k, t, d, debug = False, return_all=False):
    ...
    while iterations < k:
        ...
        test_points =  data[test_idxs]#若干行数据点（Xi,Yi)
        ...
        iterations +=1
    if bestfit is None:
        raise ValueError('did\'t meetfit acceptance criteria')
    if return_all:
        return bestfit,{'inliers':best_inlier_idxs}
    else:
        return bestfit
def random_partition(n,n_data):
    all_idxs = np.arange(n_data)#获取n_data下表索引
...
class LinearLeastSquareModel:
    ...
    def fit(self, data):
        ...
        x,resides, rank, s = sl.lstsq(A,B)#residues:残差和
        return  x #返回最小平方和向量
    def get_error(self,data,model):
        ...
        err_per_point = sp.sum((B-B_fit)**2, axis = 1)
...
def test():
    ...
    # 随机线性度，即随机生成一个斜率
    perfect_fit = 60*np.random.random(size = (n_inputs, n_outputs))
    # y = x * k
    ...
    if 1:
        ...
        A_noisy[outlier_idxs] = 20 * np.random.random((n_outliers, n_outputs))
    ...
    if 1:
        ...
        if 1:
            ...
            pylab.plot(A_noisy[non_outlier_idxs, 0], B_noisy[non_outlier_idxs, 0],'k.', label='noisy_data')
            pylab.plot(A_noisy[outlier_idxs, 0], B_noisy[outlier_idxs],'r.', label='outlier data')
        pylab.plot (A_col0_sorted[:, 0],
                  np.dot(A_col0_sorted, ransac_fit)[:, 0],
                  label='RANSAC fit')
        pylab.plot(A_col0_sorted[:, 0],np.dot(A_col0_sorted, perfect_fit)[:, 0],
                   label = 'exact system')
        pylab.plot(A_col0_sorted[:, 0],
