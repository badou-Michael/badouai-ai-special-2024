...
def detect(image_path,model_path,yolo_weights=None):
    ...
    @param model_path: 模型路劲，当使用yolo_weitghs时没有用
    @param yolo_weights:预训练模型的权重
    @return:
    """
    # ---------------------------------------#
    #   图片预处理
    ...
    resize_image = utils.resize_image(image,(416,416))
    #转成ndarray
    ...
    predictor = yolo_predict.yolo_predict(obj_threshold=config.obj_threshold,nms_threshold=config.nms_threshold,classes_file=config.classes_path,anchors_file=config.anchors_path)
    with tf.Session() as sess:
        ...
        if yolo_weights is not None:
            ...
            # 载入模型
            load_op = utils.load_weights(tf.global_variables(scope='predict'), weights_file=yolo_weights)
            sess.run(load_op)
