...
if __name__ == '__main__':
    ...
    tmp = [i-dim//2 for i in range(dim)]  # 生成一个序列 -2,-1,0,1,2。作为下面高斯核函数的x和y取值
    ...
    tmp = dim//2  # 得到2，方便下面扩展
    img_pad = np.pad(img, ((tmp, tmp), (tmp, tmp)), 'constant')  # 边缘填补，将原先图片周围填充2格，方便5*5卷积核操作
    for i in range(dx):
        for j in range(dy):
    ...
    img_tidu_x = np.zeros(img_new.shape)  # 存储梯度图像，横向处理和纵向处理维度一样
    img_tidu_y = np.zeros(img_new.shape)
    img_tidu = np.zeros(img_new.shape)  # 存储横向纵向处理后的模值
    img_pad = np.pad(img_new, ((1, 1), (1, 1)), 'constant')  # 边缘填补，此处是3*3卷积核，故填充1格
    for i in range(dx):
        for j in range(dy):
    ...
    img_tidu_x[img_tidu_x == 0] = 0.00000001  #避免分母为0
    ...
    for i in range(1, dx-1):
        for j in range(1, dy-1):
            flag = True  # 在8邻域内标记该像素值是否为极大值，true表示是极大值
            temp = img_tidu[i-1:i+2, j-1:j+2]  # 梯度幅值的8邻域矩阵
            # i = 1
            # arr = [1, 2, 3, 4, 5, 6]
            # print(arr[i - 1:i + 2])   py索引左开右闭
            # 下面判断是否真正为极大值
            if angle[i, j] <= -1:  # 使用线性插值法判断抑制与否，画图推理
    ...
    while not len(zhan) == 0:
        ...
        # 遍历栈内的强边缘像素，寻找每一个强边缘像素周围是否有介于两阈值之间的像素（弱像素值），有则说明该像素与强边缘像素相接，应该标记为边缘
    ...
    # 遍历剩下的像素点（剩下的为最外圈点以及没有与强边缘像素点连接的弱像素点），将其置为0
