...
class  NeuralNetWork:
    def __init__(self, inputnodes, hiddennodes, outputnodes, learningrate):
        ...
        初始化为节点个数的-0.5次方，使用 pow(self.hnodes, -0.5) 是一种权重初始化技巧，称为 Xavier 初始化或类似方法。
        它的目标是防止在网络深层中梯度爆炸或消失问题，确保输入和输出信号的方差大致一致
        '''
        ...
        sigmoid函数：scipy.special.expit(x)
        '''
...
for record in test_data_list:
    ...
    #预处理数字图片
    # numpy.asfarray()将输入数据转换为浮点型数组，确保后续计算的精度和一致性
    inputs = (numpy.asfarray(all_values[1:])) / 255.0 * 0.99 + 0.01
    print(type(inputs))
    #让网络判断图片对应的数字
...
# 测试单张图片的训练结果
test1 = cv2.imread('dataset/my_own_noisy_6.png')
gray_image = cv2.cvtColor(test1, cv2.COLOR_BGR2GRAY)
t = gray_image.reshape(784,1)
output = n.query(t)
label = numpy.argmax(output)
print(f'测试单张图片的识别结果为：{label}')
