...
def inputs(data_dir, batch_size, distorted):  # 这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    ...
    if distorted != None:  # 如果预处理函数中的distorted参数不为空值，就代表要进行图片增强处理
        ...
        resize_img = tf.image.resize_image_with_crop_or_pad*(reshaped_image,24,24)
        float_image = tf.image.per_image_standardization(resize_img)
        float_image.set_shape([24,24,3])
        read_input.label.set_shape([1])
        min_queue_examples = int(num_examples_per_epoch*0.4)
        image_ts,label_ts=tf.train.batch([float_image, read_input.lable], batch_size=batch_size,num_threads=16,capacity=min_queue_examples + 3 * batch_size)
        # 这里使用batch()函数代替tf.train.shuffle_batch()函数
        return image_ts, tf.reshape(label_ts, [batch_size])
