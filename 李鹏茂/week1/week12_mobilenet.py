...
def _depthwise_conv_block(inputs, pointwise_conv_filters, depth_multiplier=1, strides=(1, 1), block_id=1):
    x = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=depth_multiplier, use_bias=False, strides=strides,
                        name='depthwise_conv_{}'.format(block_id))(inputs)
    x = BatchNormalization(name='depthwise_bn_{}'.format(block_id))(x)
    x = Activation(relu6, name='depthwise_relu_{}'.format(block_id))(x)
    x = Conv2D(pointwise_conv_filters, (1, 1), padding='same', use_bias=False, strides=(1, 1),
               name='depthwise_conv2_{}'.format(block_id))(x)
    x = BatchNormalization(name='depthwise_bn2_{}'.format(block_id))(x)
    return Activation(relu6, name='depthwise_relu2_{}'.format(block_id))(x)
