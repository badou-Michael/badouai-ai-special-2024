...
def drawPrediction(image, prediction, scale_factors):
    ...
    for mask, box, label, score in zip(masks, boxes, labels, scores):
        ...
        # 从box数组中取出目标的左上角坐标，并将其转换为整数，因为坐标需要是整数才能在图像上绘制
            ...
            # 使用ImageDraw模块在矩形框的左上角上方10像素的位置绘制目标的类别标签，文本颜色设置为红色
            draw.text((top_left[0], top_left[1] - 10), str(label), fill='red')
            color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
            # 调整掩码大小以匹配原始图像
            mask_image = Image.fromarray(mask.astype(np.uint8) * 255).resize(image.size, Image.NEAREST)
            mask_image = mask_image.convert('L')
            mask_image = mask_image.point(lambda p: p > 0 and 128)  # 设置透明度为128（半透明）

            # 将掩码叠加到原始图像上
            color_image = Image.new('RGBA', image.size, color + (128,))
