...
def detect(image_path, model_path, yolo_weights=None):
    ...
    #   图片输入
    # ---------------------------------------#
    in_img = tf.placeholder(shape=[None, 416, 416, 3], dtype=tf.float32)
    in_img_shape = tf.placeholder(dtype=tf.int32, shape=(2,))
    ...
    with tf.Session() as session:
        ...
        if yolo_weights is not None:
            with tf.variable_scope('predict'):
                boxes, scores, classes = predictor.predict(in_img, in_img_shape)
            # 加载模型
            ...
            out_boxes, out_scores, out_classes = session.run(
                ...
                feed_dict={
                    ...
                    in_img: image_data,
                    # 以y、x的方式传入
                    in_img_shape: [image.size[1], image.size[0]]
                })
        else:
            boxes, scores, classes = predictor.predict(in_img, in_img_shape)
