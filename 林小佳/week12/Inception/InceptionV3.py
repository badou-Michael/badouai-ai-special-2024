...
def InceptionV3(input_shape=[299, 299, 3], classes=1000):
    ...
    x = conv2d_bn(img_input, 32, 3, 3, strides=(2, 2), padding='valid')  # padding也可以选择same
    x = conv2d_bn(x, 32, 3, 3, padding='valid')  # valid：padding=0
    ...
    # x = Flatten()(x)      # 不能加Flatten，会导致维度不匹配的报错
    x = Dense(classes, activation='softmax', name='predictions')(x)

    inputs = img_input

    model = Model(inputs, x, name='inception_v3')
    model.load_weights("inception_v3_weights_tf_dim_ordering_tf_kernels.h5")  # 加载权重
...
if __name__ == '__main__':
    ...
    x = np.expand_dims(x, axis=0)  # 维度变为(batch, H, W, C)

    x = preprocess_input(x)  # 这里不能直接调用keras.applications.imagenet_utils中的直preprocess_input函数，会导致模型泛化能力差
