...
class FRCNN(object):
    ...
    # 参数初始化/**kwargs：接受任意数量的关键字参数
    def __init__(self, **kwargs):
        self.__dict__.update(self._defaults)
        self.class_names = self._get_class()
        self.sess = K.get_session()
        self.config = Config()  # 用于存储模型的某些参数
        self.generate()  # 模型初始化
        self.bbox_util = BBoxUtility()	# 用于处理边界框相关的操作
    ...
    def generate(self):
        ...
        assert model_path.endswith('.h5'), 'Keras model or weights must be a .h5 file.'  # 确保Keras模型文件的路径以 .h5 结尾
    ...
    # 定义了一个方法，用于计算图像经过模型后的输出尺寸
    def get_img_output_length(self, width, height):
        # 定义了一个内部函数，用于计算经过卷积和池化层后的输出长度
    ...
    def detect_image(self, image):
        ...
        old_image = copy.deepcopy(image)  # copy.deepcopy 创建 image 的一个深拷贝，并存储在 old_image 变量中
        width,height = get_new_img_size(old_width,old_height)  # 接收返回的新宽度和高度
        ...
        # 图片预处理，归一化
        photo = preprocess_input(np.expand_dims(photo,0))  # 增加批处理维度
        preds = self.model_rpn.predict(photo)  # 使用区域提议网络（RPN）模型对预处理后的图像进行预测
        # 将预测结果进行解码
        ...
        for jk in range(R.shape[0]//self.config.num_rois + 1):
            ...
            [P_cls, P_regr] = self.model_classifier.predict([base_layer,ROIs])  # 使用分类器模型预测当前批次ROIs的类别和回归框
            ...
            for ii in range(P_cls.shape[1]):
                ...
                (x, y, w, h) = ROIs[0, ii, :]  # 元组解包

                cls_num = np.argmax(P_cls[0, ii, :])	
	
                (tx, ty, tw, th) = P_regr[0, ii, 4*cls_num:4*(cls_num+1)]  # 计算回归偏移量
	              # 将回归偏移量除以标准差进行缩放
        ...
        thickness = (np.shape(old_image)[0] + np.shape(old_image)[1]) // width	# 计算绘制边框时的线条粗细
        image = old_image
