...
if __name__ == '__main__':
    ...
    print("image", img)
    if pic_path[-4:] == '.png':         # .png图片在这里的存储格式是0到1的浮点数，所以扩展到255再计算
        img = img * 255                 # 还是浮点数类型
    ...
    dx, dy = img.shape
    img_new = np.zeros(img.shape)            # 存储平滑之后的图像，zeros函数得到的是浮点数型数据
    tmp = dim//2
    ...
    plt.figure(1)
    plt.imshow(img_new.astype(np.uint8), cmap='gray')       # 此时的img_new是255的浮点数型数据，强制类型转换才可以，gray灰阶
    plt.axis('off')

    # 2、求梯度，以下两个是滤波用sobel矩阵（检测图像中的水平、垂直和对角边缘）
    sobel_kernel_x = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]])
    sobel_kernel_y = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]])
    img_tidu_x = np.zeros(img_new.shape)           # 存储梯度图像
    img_tidu_y = np.zeros([dx, dy])
    img_tidu = np.zeros(img_new.shape)
    img_pad = np.pad(img_new, ((1, 1), (1, 1)), 'constant')     # 边缘填补，根据上面的矩阵结果所以写1
    for i in range(dx):
        for j in range(dy):
    ...
    for i in range(1, dx-1):
        for j in range(1, dy-1):
            ...
            temp = img_tidu[i-1:i+2, j-1:j+2]      # 地图幅值的8领域矩阵
    ...
    # 4、双阈值检测，链接边缘。遍历所有一定是边的点，查看8领域是否存在可能是边的点，进栈
    lower_boundary = img_tidu.mean() * 0.5
    high_boundary = lower_boundary * 3          # 这里我设置高阈值值是低阈值的三倍
    zhan = []
    ...
    while not len(zhan) == 0:
        ...
        if (a[0,0] < high_boundary) and (a[0,0] > lower_boundary):
            img_yizhi[temp_1-1, temp_2-1] = 255     # 这个味像素点标记为边缘
