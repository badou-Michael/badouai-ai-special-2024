...
def test():
    ...
    B_excat = sp.dot(A_exact, perfect_fit)   # y = x * k

    # 加入高斯噪声，最小二乘法能很好的处理
    A_noisy = A_exact + np.random.normal(size=A_exact.shape)  # 500*1行向量，代表Xi
    B_noisy = B_excat + np.random.normal(size=B_excat.shape)  # 500*1行向量，代表Yi
    ...
    if 1:
        ...
        B_noisy[outlier_idxs] = 50 * np.random.random(size=(n_outliers, n_outputs)) # 加入噪声和局外点的Yi
    # setup model
    all_data = np.hstack((A_noisy, B_noisy))  # 形式([Xi,Yi].....) shape:(500,2) 500行2列
    input_columns = range(n_inputs)  # 数组的第一列x:0
    output_columns = [n_inputs + i for i in range(n_outputs)]  # 数组最后一列y:1
    debug = False
    model = LinearLeastSquareModel(input_columns, output_columns, debug = debug)  # 类的实例化：用最小二乘法生成已知模型
