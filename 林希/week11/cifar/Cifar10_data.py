...
def read_cifar10(file_queue):
    ...
    """因为该数字第一个元素是标签，所以我们使用strided_slice()函数将标签提取出来，并且使用tf.cast()函数将这一个标签转换成int32的数值形式"""
    result.label = tf.cast(tf.strided_slice(record_bytes, [0], [label_bytes]),  tf.int32)

    """剩下的元素在分割出来，这些就是图片数据，因为这些数据在数据集里面存储的形式是depth * height * width。我们要把这种格式转换成[depth,height,width]"""
    """这一步是将一维数据转换成3维数据"""
    ...
    """这一步是转换数据的排布方式，变为(h,w,c)"""
    result.uint8image = tf.transpose(depth_major, [1, 2, 0])

    """返回值是已经把目标文件里面的信息都读取出来"""
    return result

"""这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作相应的操作"""
def inputs(data_dir, batch_size, distorted):
    ...
    """根据已有的文件队列使用已经定义好的文件读取函数read_cifar10()读取队列中的文件"""
    ...
    if distorted != None:
        """首先将预处理的图片进行裁剪，使用tf.random_crop()函数"""
        cropped_image = tf.random_crop(reshaped_image, [24,24, 3])
        """将裁剪好的图片进行左右翻转，使用tf.image.random_flip_left_right()函数"""
        flipped_image = tf.image.random_flip_left_right(cropped_image)
        """将左右翻转的图片进行随机亮度调整，使用tf.image.random_brightness()函数"""
        ...
        print("Filling queue with %d CIFAR iamges before starting to train.    This will take a few minutes." %min_queue_examples)
        ...
        """不对图像进行数据增强处理"""
        """这种情况下，使用函数tf.image.resize_image_with_crop_or_pad()对图片数据进行裁剪"""
        ...
        """这里使用batch替代tf.train.shuffle_batch()函数"""
