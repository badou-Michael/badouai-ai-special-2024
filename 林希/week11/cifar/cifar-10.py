...
1.使用参数w1空值L2 loss的大小
2.使用函数tf.nn.l2_loss() 计算权重L2 loss
3.使用函数tf.multiply()计算权重L2 loss 与w1的乘机，并赋值给weights_loss
4.使用函数tf.add_to_collection()将最终的结果放在名为losses的集合里面，方便后面计算神经网络的整体loss
"""
...
其中训练数据文件进行数据增强处理吗，测试数据文件不进行数据增强处理
"""
...
要注意的是，由于以后定义全连接网络的时候用到了batch_size，所以x中，第一个参数不应该是None，而应该是batc_size
"""
...
"""函数tf.nn.in_top_k() 用来计算输出结果中top k 的准确率，函数默认的k值是1，即top 1的准确率，也就是输出分类准确率最高时的数值"""
...
with tf.Session() as sess:
    ...
    for step in range(max_steps):
        ...
        if step % 100 == 0:
            ...
            print("step %d, loss=%.2d(%.1d examples/sec; %.3df sec/batch)"%(step, loss_value, examples_per_sec, sec_per_batch))

    """计算最终的正确率"""
    num_batch = int(math.ceil(num_examples_for_eval / batch_size))     # math.celi()函数用于求整
    true_count = 0
