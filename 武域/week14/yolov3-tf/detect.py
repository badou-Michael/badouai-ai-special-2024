...
def detect(image_path, model_pat, yolo_weights = None):
    """
    load model and predict image
    :param image_path: path to image
    :param model_pat: path to model
    ...
    input_image = tf.placeholder(shape = [None, 416, 416, 3], dtype = "float32", name = "input_image")

    # using yolo_predictor to predict
    ...
    with tf.Session() as sess:
        if yolo_weights is None:
            ...
            load_op = load_weights(tf.global_variables(scope='predict'), weights=yolo_weights)
            sess.run(load_op)
