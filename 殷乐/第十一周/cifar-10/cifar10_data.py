...
def read_cifar10(file_queue):
    ...
    label_bytes = 1  # 如果是Cifar-100数据集，则此处为2，这里是Cifar-10数据集
    # 分别是高宽深
    result.height = 32
    result.width = 32
    result.depth = 3
    ...
    # tf.strided_slice(record_bytes, [0], [label_bytes]）：三个参数分别是：从哪取，从第几位开始取，取到第几位截止
...
# 这对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
