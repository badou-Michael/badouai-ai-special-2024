...
def conv2d_bn(x,filters,row_num,col_num,strides=(1,1),padding='same'):
    x = Conv2D(filters,(row_num,col_num),strides=strides,padding=padding,use_bias=False)(x)
...
def inceptionV3(input_shape=[299,299,3],classes = 1000):
    ...
    x = layers.concatenate(
        [branch3x3, branch3x3dbl, branch_pool], axis=3)
