...
if __name__ =='__main__':
    ...
    print("image",img)
    if pic_path[-4:] =='.png': # .png图片在这里的存储格式是从0到1 的浮点数，要扩展到255再计算
        img = img * 255         # 还是浮点数类型
        ...
        # 2.求梯度。以下2個是滤波用的sobel矩阵(检测图像中的水平、垂直和对角边缘)
        ...
        for i in range(1,dx-1):
            for j in range(1,dy-1):
                ...
                temp = img_tidu[i-1:i+2,j-1:j+2] # 梯度幅值的8个邻域矩阵
                if angle[i,j] <=-1: #使用线性插值法判断是否抑制与否
        ...
        # 4.双阈值检测，连接边缘，遍历所有一定是边的id按，查看8邻域是否存在有可能是边的点，进栈
