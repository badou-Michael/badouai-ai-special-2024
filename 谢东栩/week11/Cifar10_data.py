...
def read_cifar10(file_queue):
    ...
    result.height = 32 #宽高，也就是像素值
    result.width = 32
    result.depth = 3  # 因为是RGB三通道，所以深度是3

    image_bytes = result.height * result.width * result.depth  # 图片样本总元素数量，像素值*3，因为三通道
...
def inputs(data_dir, batch_size, distorted):  # 这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    ...
    num_examples_per_epoch = num_examples_pre_epoch_for_train #为训练集设定样本数

    if distorted != None:  # 如果预处理函数中的distorted参数不为空值，就代表要进行图片增强处理
        cropped_image = tf.random_crop(reshaped_image, [24, 24, 3])  # 首先将预处理好的图片进行剪切，使用tf.random_crop()函数，剪切出一个24*24的图像
