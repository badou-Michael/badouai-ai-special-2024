...
def test():
    # 生成理想数据，包括输入样本x和输出样本y值
    n_samples = 500  # 样本个数
    n_inputs = 1  # 输入变量个数
    n_outputs = 1  # 输出变量个数
    # 随机生成0-20之间的500个数据，行向量,    500*1维
    A_exact = 20 * np.random.random((n_samples, n_inputs))
    # print(A_exact)  是一个500*1维的向量
    # 随机生成斜率 K ，建议符合高斯分布，乘以60
    perfect_fit = 60 * np.random.normal(size=(n_inputs,n_outputs))
    B_exact = sp.dot(A_exact, perfect_fit)  # y = x * k
    # print(B_exact)是一个500*1维的向量
    ...
    # setup model
    # 设置模型：all_data,是形式([Xi,Yi]....) shape:(500,2)500行2列
    all_data = np.hstack((A_noisy, B_noisy))
    # 输入索引 input_columns,前几列
    # 输出索引 output_columns，在输入参数基础上增加,后几列
...
class LinearLeastSquareModel:
    ...
    def __init__(self, input_columns, output_columns, debug = False):
        # input_columns:特征索引列，代码中类似于【0】，坐标
        # output_columns：输出标签列索引，代码中类似【0】，坐标
        # debug：表示什么？
    ...
    def fit(self, data):
        ...
        # A是特征列数据，即data的第0列-转为行向量，B是标签列数据，即data的第1列
...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    ...
    while iterations < k:
        # maybe_idxs = 内群点的下标序号，数组
        # test_idxs = 内群点之外的下标序号，数组
        # n - 生成模型所需的最少样本点
        ...
        print('d = ', d)
        # 当后加进来的内群点数量 大于 d时，才保留本次的模型
        if (len(also_inliers) > d):
            # maybe_inliers：最初选择的内群点, also_inliers：在计算过程中，小于误差的，后加进来的内群点
