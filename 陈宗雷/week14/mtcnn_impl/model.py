...
class MTCNN:
    def __init__(self):
        self.p_net = self.__create_net("model_data/pnet.h5",)
        self.r_net  = self.__create_net("model_data/rnet.h5")
        self.o_net = self.__create_net("model_data/onet.h5")

    @staticmethod
    def __create_net(weights_path: str, net_type: str="p_net"):
        if net_type == "p_net":
    ...
    def detect_face(self, img, threshold: List[float]):
        """
        ...
        scales = utils.scale(image)
        out = []


        #   粗略计算人脸框
        for scale in scales:
            hs = int(orig_height * scale)
            ws = int(orig_width * scale)
        ...
        rectangles = utils.filter_face_24net(cls_prob, roi_prob, rectangles, orig_width, orig_height, threshold[1])
        ...
        rectangles = utils.filter_face_48net(cls_prob, roi_prob, pts_prob, rectangles, orig_width, orig_height, threshold[2])
