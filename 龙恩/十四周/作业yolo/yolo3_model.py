...
class yolo:
    ...
    def _batch_normalization_layer(self, input_layer, name = None, training = True, norm_decay = 0.99, norm_epsilon = 1e-3):
        ...
        bn_layer = tf.keras.layers.BatchNormalization(
            ...
            scale=True, name=name
        )
        
        # 在调用时传入 training 参数
        bn_layer_output = bn_layer(input_layer, training=training)
        
        return tf.nn.leaky_relu(bn_layer_output, alpha=0.1)
    ...
    def _conv2d_layer(self, inputs, filters_num, kernel_size, name, use_bias = False, strides = 1):
        ...
        conv = tf.keras.layers.Conv2D(
            ...
            padding = ('SAME' if strides == 1 else 'VALID'), kernel_regularizer=tf.keras.regularizers.l2(5e-4), use_bias = use_bias, name = name)
        conv = conv(inputs)
        return conv
